...
import { 
  FolderKanban, Plus, Edit2, Trash2, ArrowUpRight, ArrowDownRight
} from 'lucide-react';
...
export function ProjectCategoryManagement() {
  const { t } = useI18n();
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await saveProjectCategory(payload, editingCategory?.id);
  ...
  const handleToggle = async (cat: ProjectCategory) => {
    try {
      await toggleProjectCategory(cat.id, cat.is_active || false);
      load();
      refreshAll();
    } catch {}
  };
  ...
  const renderList = (items: ProjectCategory[]) =>
    items.map(cat => (
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingCategory ? t('projectCategory.edit') : t('projectCategory.add')}</DialogTitle>
            </DialogHeader>
      ...
      <CardContent>
        <p className="text-sm text-muted-foreground mb-4">{t('projectCategory.description')}</p>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            <h3 className="flex items-center gap-2 font-medium mb-3">
              <ArrowUpRight className="w-4 h-4 text-success" />
              {t('projectCategory.incomeCategories')} ({incomeCategories.length})
            </h3>
            <div className="space-y-2">
              {incomeCategories.length === 0
                ? <p className="text-sm text-muted-foreground py-4 text-center">{t('projectCategory.noIncome')}</p>
          ...
          <div>
            <h3 className="flex items-center gap-2 font-medium mb-3">
              ...
              {t('projectCategory.expenseCategories')} ({expenseCategories.length})
            </h3>
            <div className="space-y-2">
              {expenseCategories.length === 0
                ? <p className="text-sm text-muted-foreground py-4 text-center">{t('projectCategory.noExpense')}</p>
