...
export default function Exchange() {
  ...
  // 使用 service hook 替代直接 supabase 调用
  const { data: rawExchanges = [], isLoading } = useExchangeTransactions(
    tenantId,
    {
  ...
  const exchanges = rawExchanges as unknown as ExchangeTransaction[];

  const handleEdit = (exchange: ExchangeTransaction) => {
  ...
  // 计算统计数据
  const totalProfit = exchanges
    .filter(e => e.profit_loss > 0)
    .reduce((sum, e) => sum + e.profit_loss, 0);
  
  const totalLoss = exchanges
    .filter(e => e.profit_loss < 0)
    .reduce((sum, e) => sum + Math.abs(e.profit_loss), 0);
  ...
  const filteredExchanges = exchanges.filter(e => {
    ...
    return (
      e.remark?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      e.sequence_no.toString().includes(searchTerm)
    );
  });
  ...
  return (
    <MainLayout>
      <div className="animate-page-enter space-y-5">
        ...
        <CurrencyStatsPanel 
          stats={currencyStats} 
          title={t('exchange.currencyStats')}
          showBalance={true}
          defaultOpen={false}
        />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <div className="stat-card-v2 group">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 bg-success/10 transition-transform group-hover:scale-110">
                <TrendingUp className="w-5 h-5 text-success" />
              </div>
              <div className="min-w-0">
                <p className="text-lg sm:text-xl font-bold text-success tabular-nums leading-tight break-all">
                  +RM {totalProfit.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                </p>
          ...
          <div className="stat-card-v2 group">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 bg-destructive/10 transition-transform group-hover:scale-110">
                <TrendingDown className="w-5 h-5 text-destructive" />
              </div>
              <div className="min-w-0">
                <p className="text-lg sm:text-xl font-bold text-destructive tabular-nums leading-tight break-all">
                  -RM {totalLoss.toLocaleString('zh-CN', { minimumFractionDigits: 2 })}
                </p>
          ...
          <div className="stat-card-v2 group">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0 bg-primary/10 transition-transform group-hover:scale-110">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Select value={currencyFilter} onValueChange={setCurrencyFilter}>
                <SelectTrigger className="w-full sm:w-[150px]">
                  <SelectValue placeholder={t('exchange.allCurrencies')} />
                </SelectTrigger>
                <SelectContent>
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editingExchange ? t('exchange.editExchange') : t('exchange.newExchange')}
              </DialogTitle>
            </DialogHeader>
            <ExchangeForm
