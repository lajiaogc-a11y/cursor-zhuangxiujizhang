...
export default function Payroll() {
  ...
  const { data: advances = [] } = useQuery({
    ...
    queryFn: () => fetchSalaryAdvances(tenantId!),
    enabled: !!tenantId,
  });

  const { data: payments = [] } = useQuery({
    queryKey: [...queryKeys.salaryPayments, tenantId],
    queryFn: () => fetchSalaryPayments(tenantId!),
    enabled: !!tenantId,
  });

  const { data: insurances = [] } = useQuery({
    queryKey: [...queryKeys.insurancePayments, tenantId],
    queryFn: () => fetchInsurancePayments(tenantId!),
    enabled: !!tenantId,
  });
  ...
  const invalidatePayroll = () => {
    invalidationMap.payrollMutation.forEach(key => {
  ...
  const stats = useMemo(() => {
    ...
    const monthlySalaryTotal = monthlyPayments.reduce((sum, p) => sum + p.amount_myr, 0);
    const monthlyAdvanceTotal = monthlyAdvances.reduce((sum, a) => sum + a.amount_myr, 0);
    const monthlyInsuranceTotal = monthlyInsurance.reduce((sum, i) => sum + i.amount_myr, 0);
    const activeEmployees = employees.filter(e => e.status === 'active');
    const totalMonthlySalary = activeEmployees.reduce((sum, e) => sum + e.monthly_salary, 0);
    const pendingSalary = Math.max(0, totalMonthlySalary - monthlySalaryTotal);
    return { monthlySalary: monthlySalaryTotal, monthlyAdvance: monthlyAdvanceTotal, monthlyInsurance: monthlyInsuranceTotal, pendingSalary, employeeCount: activeEmployees.length };
    ...
    <MainLayout>
      <div className="animate-page-enter space-y-5">
        <div className="flex justify-between items-center">
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">{t('payroll.title')}</h1>
          <div className="flex gap-2">
            {canEdit && (
              isMobile ? (
                <Button variant="outline" size="sm" onClick={() => { setEditingEmployee(null); setEmployeeFormOpen(true); }}>
                  <Plus className="w-4 h-4" />
                </Button>
              ) : (
                <Button variant="outline" onClick={() => { setEditingEmployee(null); setEmployeeFormOpen(true); }}>
                  <Plus className="w-4 h-4 mr-2" />{t('payroll.newEmployee')}
                </Button>
              )
            )}
          </div>
          ...
          <CardContent className="pt-6">
            <Tabs defaultValue="salary">
              ...
              <TabsContent value="salary">
                <SalaryCalculation employees={employees} loading={isLoading} onRefresh={invalidatePayroll} />
              </TabsContent>
              ...
              <TabsContent value="history">
                <PaymentHistory payments={paymentsWithEmployee} loading={isLoading} onEdit={(pay) => { setEditingPayment(pay); setPaymentFormOpen(true); }} onRefresh={invalidatePayroll} canEdit={canEdit} />
              </TabsContent>
