...
export function useQProducts() {
  ...
  const toggleFavorite = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['q_product_favorites'] }),
  });

  const addProduct = useMutation({
    mutationFn: (product: Omit<Product, 'id'>) => qs.addProduct(product, user?.id, tenant?.id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['q_products'] }); toast({ title: '产品已添加' }); },
  ...
  const updateProduct = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['q_products'] }); toast({ title: '产品已更新' }); },
    onError: (e: any) => toast({ title: '更新失败', description: e.message, variant: 'destructive' }),
  });

  const deleteProduct = useMutation({
    mutationFn: (id: string) => qs.deleteProduct(id),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['q_products'] }); toast({ title: '产品已删除' }); },
